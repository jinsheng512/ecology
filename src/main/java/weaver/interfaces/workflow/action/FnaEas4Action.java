 ...
 *
 */
public class FnaEas4Action implements Action{
	...
	public String execute(RequestInfo request) {
		...
		try {
			bb.writeLog("FnaEas4Action:"+request.getRequestid());
			...
			String tableAmount = Util.null2String(bb.getPropValue("QC228542", "table_"+formIdInt+"_amount"));//主表字段,申请金额
			String tableCompany = Util.null2String(bb.getPropValue("QC228542", "table_"+formIdInt+"_company"));//主表字段,现金支付公司
			...
			String sql1 = "select a."+tableNumber+" as tableNumber,"
								  ...
								  + " b.lastname "
								  + " from "+miantable+" a "
								  + " join hrmresource b on a."+tablePayee+" = b.id "
								  + " where a.requestid = "+requestid;
			bb.writeLog("sql1>>>>", sql1);
			boolean sqlFlag = rs.executeSql(sql1);
			if(!sqlFlag){
				throw new Exception("sql出错，位置FnaEas4Action-001");
			}
			...
			map1.put("payBillType", "212");
			map1.put("description","付"+lastnamePayee+"转账款");
