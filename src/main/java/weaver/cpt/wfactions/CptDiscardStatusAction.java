 ...
 *
 */
public class CptDiscardStatusAction implements Action,Runnable {
	...
	public String execute(RequestInfo request) {
		...
		try {
			...
			baseBean.writeLog("wfObject:"+ wfObject.toString());
					...
					if (detailtable.length > dtnum) {
						...
						for (int j = 0; j < s.length; j++) {
							...
							for (int k = 0; k < c.length; k++) {
								...
								baseBean.writeLog("detail:"+ "name:"+name+",value:"+value);
								
//							
								if (name.equalsIgnoreCase("shifycl")) {
									shifycl=value;
...
//							
							jsonObject.put("zc", zc);
//							
							jsonObject.put("shifycl", shifycl);
							
							jsonArray.put(jsonObject);
							
							baseBean.writeLog("jsonArray:"+ "jsonArray.toString():"+jsonArray.toString()+",length:"+jsonArray.length());
				...
				for(int i=0;i<jsonArray.length();i++){
					...
					baseBean.writeLog("111111111111");
...
//					jg=jsonObject.getString("jg");
					...
					shifycl=jsonObject.getString("shifycl");
					
					
                    RecordSet.executeSql("update CptCapital set shifycl='"+shifycl+"' where id="+zic);
