 ...
 *
 */
public class CptDiscardAction implements Action,Runnable {
		...
		synchronized (lock) {
			...
			try {
				...
				for(int i=0;i<jsonArray.length();i++){
					...
					sptcount=Util.null2String(new CptDwrUtil().getCptInfoMap(zic).get("sptcount"));
					if ("".equals( sptcount)){
						sptcount="0" ;
					}
					...
					if(sptcount.equals("1")){
						...
						para +=separator+"";
						para +=separator+""+Util.getDoubleValue(jg,0.0);
						para +=separator+"5";
						para +=separator+bz;
						...
						para +=separator+shenqbm;
						para +=separator+shenqr;
						para +=separator+""+Util.getDoubleValue(shul,0.0);
						para +=separator+location;
						para +=separator+requestid;
						para +=separator+"";
						para +=separator+""+Util.getDoubleValue(jg,0.0);
						...
						{
					    	request.getRequestManager().setMessageid("20088");
				        	request.getRequestManager().setMessagecontent(CapitalComInfo.getCapitalname(zic)+" 数量不够!");
