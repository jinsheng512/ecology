...
public class SetApprovalStatus extends BaseBean implements Action {
	
	private static BaseBean baseBean=new BaseBean();

   private static Log log = LogFactory.getLog(SetApprovalStatus.class.getName());
   ...
   public String execute(RequestInfo requestinfo) {
	...
	baseBean.writeLog("tagtag run action1审批结算单 :"+getClass()+",requestid:"+requestinfo.getRequestid()+",wfid:"+requestinfo.getWorkflowid()+",src:"+requestinfo.getRequestManager().getSrc());
		 rs = new RecordSet();
...
//	String sqlString = "select * from ( select * from workflow_requestlog where requestid="+rid+""+" order by  operatedate desc ,operatetime desc) where rownum=1";
	 ...
	 // 转换为键值对  
        String params ="appkey="+"qibei"+"&clearno="+danjbh+"&operate="+status+"&remark="+remark;
     
		baseBean.writeLog("tagtag run action3 :"+"参数:"+params);
		
		JsonObject isok =HttpRequestUtil.postJson("http://192.168.100.131:8888/api/fin/loan/oaClearApprove", params, true);
//	
