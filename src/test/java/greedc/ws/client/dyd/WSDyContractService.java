...
public class WSDyContractService {

    private static final Logger logger = Logger.getLogger(WSDyContractService.class);
    ...
    public static void main(String[] args) {
        ...
        WSDyContractService dydservice = new WSDyContractService(danju, dydOADanjuSpyjParamsList, dydOADanjuHtmxParamsList,dydFlowSpmxParamsList);

        String rs = dydservice.CallOAFlow();
        System.out.println("抵押贷OA流程推送结果: " + rs);
//        HKDYDOrderInfo dydorder = HKDYDOrderInfo.dao.findById(666);
//        dydorder.set("oaflowid", 666);
//        dydorder.save();
    ...
    public WSDyContractService(DydOADanjuParams dydOADanjuParams, List<DydOADanjuSpyjParams> dydOADanjuSpyjParamsList,
    ...
    public String CallOAFlow() {
        ...
        try {
            ...
            //CustomWorkflowRequestInfo requestInfo = new WSDyContractService().getWorkflowRequestInfo();
    ...
    private WorkflowRequestTableField[] getWorkflowRequestTableField() {
        ...
        fieldMap.put("suosgs", "77");
        fieldMap.put("suosbm", "408");
        fieldMap.put("shenqr", "6600");
        ...
        String fjurl = this.getDanjuParams().getFuj();
        if (StringUtils.isNotEmpty(fjurl)) {
            String [] fjurls = fjurl.split(",");
            String allfjurl = "";
            for(int i=0;i<fjurls.length;i++){
                if(i==0)
                    allfjurl = "<a href='http://192.168.100.131:8890upload/temp/" + fjurls[i] + "' target='_blank'>风控综合附件"+i+"</a>";
                else
                    allfjurl += "&nbsp;&nbsp;" + "<a href=' http://192.168.100.131:8890/upload/temp/" + fjurls[i] + "' target='_blank'>风控综合附件"+i+"</a>"; 
            }
            logger.info("风控综合附件：" + allfjurl);
            fieldMap.put("fuj", allfjurl); // 身份证正面
        }
    ...
    private ArrayOfWorkflowRequestTableRecord getWorkflowRequestTableRecords() {
        ...
        if (this.getDydOADanjuSpyjParamsList() != null && this.getDydOADanjuSpyjParamsList().size() > 0) {
            System.out.println("风控审批明细数据条数: " + this.getDydOADanjuSpyjParamsList().size());
            ...
            for (int i = 0; i < this.getDydOADanjuSpyjParamsList().size(); i++) {
                ...
                String fujian = SpyjParams.getFujzl();
                if (StringUtils.isNotEmpty(fujian)) {
                    fujian = fujian.replace("\\", "/");
                    fujian = "<a href='" + fujian + "' target='_blank'>附件资料</a>";
                    logger.info("风控附件路径：" + fujian);
                    fieldMap.put("fujzl", fujian); // 附件
                }
    ...
    private ArrayOfWorkflowRequestTableRecord getWorkflowRequestTableRecords1() {
        ...
        if (this.getDydOADanjuHtmxParamsList() != null && this.getDydOADanjuHtmxParamsList().size() > 0) {
            System.out.println("审批合同明细数据条数: " + this.getDydOADanjuHtmxParamsList().size());
            ...
            for (int i = 0; i < this.getDydOADanjuHtmxParamsList().size(); i++) {
                ...
                String hetdzb = HtmxParams.getHetdzb();
                if (StringUtils.isNotEmpty(hetdzb)) {
                    hetdzb = hetdzb.replace("\\", "/");
                    logger.info("合同路径：" + hetdzb);
                    hetdzb = "<a href='" + hetdzb + "' target='_blank'>合同</a>";
                    fieldMap.put("hetdzb", hetdzb); // 合同
                }
