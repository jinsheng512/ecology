$(document).ready(function(){
	...
	var FN = {
		...
		sumRows: function(projectName, area, productType, roomModel) {
		...
		generateRows_roomModel: function(projectName, area, productType) {
			var rowOption = {};
			rowOption.html = '';
			rowOption.count = 0;
			var roomModels = DATA.projectNames[projectName][area][productType];
			for (var i = 0; i < roomModels.length; i++) {
				var row = DATA.rows[projectName + area + productType + roomModels[i]];
				row.avgPrice = divide(row.salesAmount, row.salesArea, 0);
				rowOption.html += FN.generateRow(rowOption.html != '', 'td', roomModels[i], '', row);
				rowOption.count++;
				FN.sumRows(projectName, area, productType, roomModels[i]);
			}
		...
		generateRows_productType: function(projectName, area) {
			...
			for (var productType in productTypes) {
				var row = FN.generateRows_roomModel(projectName, area, productType);
